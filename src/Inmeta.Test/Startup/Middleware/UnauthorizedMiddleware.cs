...
namespace Inmeta.Test.Startup.Middleware
{
    internal sealed class UnauthorizedMiddleware
    {
        ...
        private ProblemDetails ProblemDetails(HttpContext context)
        {
            var problemDetails = ProblemDetailsFactory.Create(
				...
				"Please follow one of the 'auth_urls' to authenticate with the Inmeta.Test Orders API - choose 'cookie_session_url' for 'Cookie' scheme (browser), 'tokenUrl' for 'Bearer' scheme (access_token)"
			);
