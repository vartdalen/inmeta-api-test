...
using Inmeta.Test.OAuth.Api.Models.Abstractions;
using Inmeta.Test.OAuth.Api.Refit;
using Inmeta.Test.OAuth.Extensions;
...
namespace Inmeta.Test.OAuth.Api.Controllers
{
    ...
    public class OAuthController : ControllerBase
    {
		...
		private async Task<IActionResult> RefreshToken([FromBody] TokenRequest request)
		{
            ...
            if (exPrincipal.FindFirst(p => p.Type == "customer_hash_id")?.Value != userId) return Unauthorized();
        ...
        )
		{
			...
			var name = acPrincipal.FindFirst(p => p.Type == "name")?.Value;
			if (string.IsNullOrEmpty(email)) { return BadRequest($"Registration failed, token is missing required claim type: '{ClaimTypes.Email}'"); }
			if (string.IsNullOrEmpty(name)) { return BadRequest($"Registration failed, token is missing required claim type: 'name'"); }
			...
			catch (KeyNotFoundException) { createCustomerResult = await _customerService.Create(new CreateCustomerDto { Email = email, Name = name }); }
			var hashId = readCustomerResult?.HashId ?? createCustomerResult?.HashId;
			if (hashId is null) return StatusCode(500);
			acPrincipal.AddClaimsToDeepCopy(_config, out var atClaims, hashId);
			...
			InMemoryTokenStore.AddRefreshToken(hashId, user_refresh_token);
			...
			return SuccessResult(createCustomerResult, email, hashId);
		}
